...
struct FzStextPage
{
	...
	*/
	...
	/**
		...
		the table.
	*/
	FZ_FUNCTION FzStextBlock fz_find_table_within_bounds(const FzRect& bounds, FzTableHuntOptions& opts) const;
	...
	/**
		...
		limit.
	*/
	FZ_FUNCTION FzStextBlock fz_find_table_within_grid(const FzStextGridPositions& xpos, const FzStextGridPositions& ypos, float limit, FzTableHuntOptions& opts) const;

	/** Class-aware wrapper for `::fz_find_table_within_grid_dividers()`.  */
	FZ_FUNCTION FzStextBlock fz_find_table_within_grid_dividers(const std::vector<fz_stext_grid_divider> &xs, const std::vector<fz_stext_grid_divider> &ys, float limit, FzTableHuntOptions& opts) const;

	/** Class-aware wrapper for `::fz_find_table_within_grid_floats()`.  */
	/** Swig-friendly wrapper for fz_find_table_within_grid(). */
	FZ_FUNCTION FzStextBlock fz_find_table_within_grid_floats(const std::vector<float> &xs, const std::vector<float> &ys, float limit, FzTableHuntOptions& opts) const;
	...
	*/
	...
	This method has out-params. Python/C# wrappers look like:
		`fz_propose_table_within_bounds(::fz_rect bounds, ::fz_stext_grid_positions **xposp, ::fz_stext_grid_positions **yposp, const ::fz_table_hunt_options *opts)` => `(int)`
	 */
	/**
		...
		freed.
	*/
	FZ_FUNCTION int fz_propose_table_within_bounds(const FzRect& bounds, FzStextGridPositions& xposp, FzStextGridPositions& yposp, FzTableHuntOptions& opts) const;
	...
	*/
	...
	/**
		...
		information.
	*/
	FZ_FUNCTION void fz_table_hunt(FzTableHuntOptions& opts) const;
	...
	*/
	FZ_FUNCTION void fz_table_hunt_within_bounds(const FzRect& bounds, FzTableHuntOptions& opts) const;
...
/** Wrapper class for struct `fz_table_hunt_options`. */
/**
...
{
	/** Construct */
	FZ_FUNCTION FzTableHuntOptions(int vertically_collapse_bordered_cells);

	/** We use default copy constructor and operator=. */

	/** Default constructor, sets each member to default value. */
	FZ_FUNCTION FzTableHuntOptions();

	/* == Methods. */

	/** Class-aware wrapper for `::fz_apply_table_hunt_options()`.  */
	/**
		Parse table hunt device options from an fz_options struct
		into an already initialised opts structure.
	*/
	FZ_FUNCTION void fz_apply_table_hunt_options(const FzOptions& options);

	/** Class-aware wrapper for `::fz_init_table_hunt_options()`.  */
	FZ_FUNCTION void fz_init_table_hunt_options();

	/** Class-aware wrapper for `::fz_parse_table_hunt_options()`.  */
	/**
		...
		This initialises the opts structure.
	*/
	FZ_FUNCTION FzTableHuntOptions fz_parse_table_hunt_options(const char *args);

	/** Constructor using raw copy of pre-existing `::fz_table_hunt_options`. */
	FZ_FUNCTION FzTableHuntOptions(const ::fz_table_hunt_options* internal);

	/** Constructor using raw copy of pre-existing `::fz_table_hunt_options`. */
	FZ_FUNCTION FzTableHuntOptions(const ::fz_table_hunt_options internal);

	/** Access as underlying struct. */
	FZ_FUNCTION ::fz_table_hunt_options* internal();

	/** Access as underlying struct. */
	FZ_FUNCTION const ::fz_table_hunt_options* internal() const;
	...
	/* These members are the same as the members of ::fz_table_hunt_options. */
	...
	/** Comparison method. */
	FZ_FUNCTION bool operator==(const FzTableHuntOptions& rhs);
...
struct PdfPage
{
	...
	FZ_FUNCTION void pdf_clip_page(const FzRect& clip) const;
...
/** mupdf::FzTableHuntOptions: writes underlying fz_table_hunt_options's members, labelled and inside (...), to a stream. */
FZ_FUNCTION std::ostream& operator<< (std::ostream& out, const mupdf::FzTableHuntOptions& rhs);

/** mupdf::FzTableHuntOptions: comparison function. */
FZ_FUNCTION bool operator==( const mupdf::FzTableHuntOptions& lhs, const mupdf::FzTableHuntOptions& rhs);
